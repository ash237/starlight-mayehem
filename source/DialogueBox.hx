...
class DialogueBox extends FlxSpriteGroup
{
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		blackness = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.BLACK);
		add(blackness);
		...
		switch (PlayState.SONG.song.toLowerCase()) {
			case 'echoes' | 'inverted-ascension' | 'artificial-lust':
				box.animation.addByPrefix('left', 'left', 24, true);
				box.animation.addByPrefix('right', 'right', 24, true);
		...
		switch (PlayState.SONG.song.toLowerCase()) {
			case 'echoes' | 'inverted-ascension' | 'artificial-lust':
				swagDialogue.x -= 100;
				swagDialogue.y += 5;
				swagDialogue.width = FlxG.width * 0.8;
				dropText.width = FlxG.width * 0.8;
				dropText.x -= 100;
				dropText.y += 5;
				swagDialogue.font = Paths.font("starlight.otf");
				dropText.font = Paths.font("starlight.otf");
		...
		charText = new FlxText(840, 405, Std.int(FlxG.width * 0.6), "", Std.parseInt(curFontScale));
		charText.font = Paths.font("pfont.ttf");
		charText.color = FlxColor.fromString('#FF000000');
		add(charText);
		charText.text = curCharText;
		charText.size = 70;
			...
			if (box.animation.curAnim.name == 'open' && box.animation.curAnim.finished)
			{
				switch (PlayState.SONG.song.toLowerCase()) {
					case 'echoes' | 'inverted-ascension' | 'artificial-lust':
						...
						if (curFlip == 'true')
							box.animation.play('left');
						else
							box.animation.play('right');
						dialogueOpened = true;
						box.visible = true;
					default:
			...
			if (dialogueList[1] == null && dialogueList[0] != null)
			{
				if (!isEnding)
				{
					...
					new FlxTimer().start(fadeOutTime, function(tmr:FlxTimer)
					{
						...
						charText.alpha = swagDialogue.alpha;
						cutscene.alpha = charText.alpha;
					}, fadeOutLoop);
	...
	function startDialogue():Void
	{
		...
		if (curSong != '') {
			FlxG.sound.music.fadeOut(2.2, 0);
			FlxG.sound.playMusic(Paths.music('dialogueMusic/'+curSong), 0);
		}
		...
		charText.color = FlxColor.fromString(charColor);

		if (oldCutscene != curCutscene) {
			remove(swagDialogue);
			remove(charText);
			inCutscene = true;
			new FlxTimer().start(1, function(daTmr:FlxTimer)
			{
				inCutscene = false;
				add(box);
				add(swagDialogue);
		...
		switch (PlayState.SONG.song.toLowerCase()) {
			case 'echoes' | 'inverted-ascension' | 'artificial-lust':
				dropText.font = swagDialogue.font = Paths.font("starlight.otf");
			default:
