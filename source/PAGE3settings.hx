	...
	function changePress(Change:Int = 0)
		{
			switch (optionShit[curSelected])
			{
                ...
                case "nps":
                    ...
                    FlxG.sound.play(Paths.sound('scrollMenu'), _variables.svolume/100);
                case "iconZoom":
                    _variables.iconZoom += FlxMath.roundDecimal(Change/4, 2);
                    if (_variables.iconZoom < 0)
                        _variables.iconZoom = 0;
    
                    FlxG.sound.play(Paths.sound('scrollMenu'), _variables.svolume/100);
                case "cameraZoom":
                    _variables.cameraZoom += FlxMath.roundDecimal(Change/4, 2);
                    if (_variables.cameraZoom < 0)
                        _variables.cameraZoom = 0;
        
                    FlxG.sound.play(Paths.sound('scrollMenu'), _variables.svolume/100);
                case "cameraSpeed":
                    _variables.cameraSpeed += FlxMath.roundDecimal(Change/2, 2);
