...
class Character extends FlxSprite
{
	...
	public var iconColor:String = '#FF31B0D1';

	public var holdTimer:Float = 0;
	var rgbString:String = '';

	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false, ?isRGB:Bool = false)
	{
		super(x, y);
		...
		{
			var data:Array<String> = initColorList[i].split(':');
			if (data[0] == character)
				iconColor = data[1];
		}
		...
		switch (curCharacter)
		{
			...
			case 'nogf':
				// GIRLFRIEND CODE
				tex = Paths.getSparrowAtlas('characters/NoGF' + rgbString,'shared');
				frames = tex;
				animation.addByPrefix('cheer', 'GF Cheer', 24, false);
				animation.addByPrefix('singLEFT', 'GF left note', 24, false);
				animation.addByPrefix('singRIGHT', 'GF Right Note', 24, false);
			...
			case 'bf':
				...
				flipX = true;
			case 'bfgf':
				var tex = Paths.getSparrowAtlas('characters/BFGF' + rgbString,'shared');
				frames = tex;
				animation.addByPrefix('idle', 'BF idle dance', 24, false);
				animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
				animation.addByPrefix('singLEFT', 'BF NOTE LEFT0', 24, false);
			...
			case 'ruby':
				...
				playAnim('idle');
			case 'max':
				tex = Paths.getSparrowAtlas('characters/Max' + rgbString,'shared');
				frames = tex;
				...
				playAnim('idle');
			case 'abel':
				tex = Paths.getSparrowAtlas('characters/Abel' + rgbString,'shared');
				frames = tex;
				...
				playAnim('idle');
			case 'duet':
				tex = Paths.getSparrowAtlas('characters/duet_assets' + rgbString,'shared');
				frames = tex;
	...
	override function update(elapsed:Float)
	{
		if (!curCharacter.startsWith('bf'))
		{
			...
			if (curCharacter == 'dad')
				dadVar = 6.1;
			/*if (curCharacter == 'duet' && rgbString != '') {
				if (animation.curAnim.name.startsWith('sing')) {
					playAnim('idle');
				}
			}
		...
		switch (curCharacter)
		{
			case 'gf':
				if (animation.curAnim.name == 'hairFall' && animation.curAnim.finished)
					playAnim('danceRight');
			case 'cj':
				if (animation.curAnim.finished && animation.curAnim.name == 'intro') {
					playAnim('idle');
				}
		}
	...
	public function dance()
	{
		if (!debugMode)
		{
			switch (curCharacter)
			{
				case 'gf' | 'gf-christmas' | 'gf-car' | 'gf-pixel' | 'nogf':
				...
				case 'duet':
					if (!animation.curAnim.name.startsWith('showtime'))
					{
						playAnim('idle');
					}
				case 'spooky':
					...
					if (danced)
						...
						playAnim('danceLeft');
				case 'cj':
					if (!animation.curAnim.name.startsWith('intro'))
					{
						playAnim('idle');
					} 
				default:
